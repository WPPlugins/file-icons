...
function dprx_fileicons_save_pattern($pattern,$file) {
	...
	foreach($icons as $icon) {
		if ($icon['pattern'] == $pattern) {
			$updated_iconset[] = array("pattern" => $pattern,"icon" => $file); $updated = 1;
		} else {
			$updated_iconset[] = $icon; 
		}
	}
	if (empty($updated)) {
			$updated_iconset[] = array("pattern" => $pattern,"icon" => $file);
	}
	...
	<h2><?php _e("File-Icons Settings","dprx_fileicons"); ?></h2>
	  ...
	  <input type="submit" class="button" value="<?php _e("Save","dprx_fileicons"); ?>" />
		...
		?><tr <?php if ($i == 1) { echo "style=\"background-color:#eeeeee\""; } ?>>
			<td><?php echo $icon['pattern']; ?></td>
			<td><img src="<?php bloginfo("wpurl"); ?>/wp-content/plugins/file-icons/icons/<?php echo $iconset; ?>/<?php echo $icon['icon']; ?>" /></td>
			<td><a href="<?php bloginfo("wpurl"); ?>/wp-admin/admin.php?page=<?php echo $_REQUEST['page']; ?>&amp;dprx_fileicons_edit_pattern=<?php echo urlencode($icon['pattern']); ?>&amp;dprx_fileicons_edit_file=<?php echo urlencode($icon['icon']); ?>"><?php _e("edit","dprx_fileicons"); ?></a>
				<a href="<?php bloginfo("wpurl"); ?>/wp-admin/admin.php?page=<?php echo $_REQUEST['page']; ?>&amp;dprx_fileicons_delete_pattern=<?php echo urlencode($icon['pattern']); ?>"><?php _e("delete","dprx_fileicons"); ?></a>
			</td>
		 </tr>
		<?php
		}
